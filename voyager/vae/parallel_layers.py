...
class PatchGroupNorm3d(BaseModule):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        size = torch.full((1, self.module.num_groups, 1, 1, 1, 1), x.shape[self.split_dim] - 1, dtype=dtype,
                          device=x.device)
...
class PatchCausalConv3d(BaseModule):
    def __init__(self, module: nn.Conv3d, split_dim, num_blocks=2):
        ...
        assert self.module.conv.kernel_size[-2] == 3 or self.module.conv.kernel_size[
            -2] == 1, "PatchCausalConv3d only support kernel_size (3, 3, 3) or (1, 1, 1)"
    ...
    def forward(self, x):
        ...
        if (world_size == 1) or (self.module.conv.kernel_size[-2] == 1):
            ...
            if self.split_dim == -1:
                ...
                raise ValueError(
                    f"Cannot split video sequence into ulysses_degree x ring_degree ({world_size}) parts evenly")
            ...
            for o in range(2):
                ...
                x_o = self._slice(x, size_out - end_o, size_out - start_o, dim_out) if rank % 2 == 0 else self._slice(x,
                                                                                                                      start_o,
                                                                                                                      end_o,
                                                                                                                      dim_out)
